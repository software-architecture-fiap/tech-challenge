...
def update_order_status(
    ...
    logger.info(
        f"Update order status endpoint called for order ID: {order_id} "
        f"with status: {update_data.status}"
    )
    try:
    ...
    try:
        db_order = repository.update_order_status(
            db, order_id=order_id_int, status=update_data.status
        )
        if db_order is None:
            logger.warning(f"Order not found: {order_id}")
            raise HTTPException(status_code=404, detail="Order not found")
        logger.info(
            f"Order ID {order_id} status updated to {update_data.status}"
        )
        return db_order
    except Exception as e:
